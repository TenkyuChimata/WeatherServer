...
def get_data():
    try:
        ...
        return None
    return temperature, humidity, pressure, radiation, radiation_avg, pm25, pm10, create_at
...
def plot(x, y, y_name, plot_name, html_name):
    line = (
        ...
        .set_global_opts(title_opts = opts.TitleOpts(title = plot_name), xaxis_opts = opts.AxisOpts(axislabel_opts = opts.LabelOpts(rotate = 90, interval = 0, is_show = False)), yaxis_opts = opts.AxisOpts(is_scale = True))
    )
...
while True:
    try:
        ...
        if weather_data != None:
            ...
            plot(list(createat_list), list(temperature_list), "温度 (℃)", "温度", "/var/www/html/temperature.html")
            plot(list(createat_list), list(humidity_list), "湿度 (%RH)", "湿度", "/var/www/html/humidity.html")
            plot(list(createat_list), list(pressure_list), "大气压 (hPa)", "大气压", "/var/www/html/pressure.html")
            plot(list(createat_list), list(radiation_list), "电离辐射 (μSv/h)", "电离辐射", "/var/www/html/radiation.html")
            plot(list(createat_list), list(radiation_avg_list), "电离辐射 (μSv/h)", "电离辐射(小时均值)", "/var/www/html/radiation_avg.html")
